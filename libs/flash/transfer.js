...
function prepare(settlement, stakes, deposits, fromIndex, destinations) {
  ...
  transfer.find(tx => tx.address == settlement[fromIndex]).value -=
    stakes[fromIndex] * total
...
function compose(
  ...
  const valueTransfersLength = transfers.filter(transfer => transfer.value < 0)
    .length
  let stateCopy = {
  ...
  if (
    valueTransfersLength != 0 &&
    valueTransfersLength > stateCopy.stakes.length
  ) {
  ...
  for (let i = 0; i < multisigs.length - 1; i++) {
    if (
      multisigs[i].bundles.find(bundle =>
        bundle.find(tx => tx.value > 0 && tx.address == remainder)
      )
    ) {
  ...
  multisigs.slice(0, multisigs.length - 1).map((multisig, i) => {
    ...
    bundles.push(
      IOTACrypto.multisig.initiateTransfer(input, remainderAddress, transfers)
    )
  })
  ...
  bundles.push(
    IOTACrypto.multisig.initiateTransfer(input, remainder.address, transfers)
  )
  return bundles
}
    ...
    transfers.filter(
      transfer =>
        transfer
          .filter(tx => tx.value < 0)
          .filter(
            tx => !IOTACrypto.utils.validateSignatures(transfer, tx.address)
          ).length != 0
    ).length != 0
  ) {
...
function getDiff(root, remainder, history, bundles) {
  ...
  const initialInputTransaction = bundles[0].filter(
    bundle => bundle.value < 0
  )[0]
  ...
  const initialIndex = multisigs
    .filter(m => m.address == initialInputTransaction.address)
    .map((m, i) => i)[0]
  for (
    let i = initialIndex;
    i < multisigs.length - 1 && i - initialIndex < bundles.length - 1;
    i++
  ) {
    const bundle = bundles[i - initialIndex]
    const inputTransaction = bundle.filter(tx => tx.value < 0)[0]
    if (
      typeof inputTransaction === undefined ||
      inputTransaction.address != multisigs[i].address
    ) {
    ...
    if (
      bundle.find(tx => tx.value > 0 && tx.address != multisigs[i + 1].address)
    ) {
  ...
  const previousRemainder = previousTransfer
    .filter(tx => tx.address == remainder.address && tx.value > 0)
    .reduce((acc, v) => acc + v, 0)
  const newRemainder = lastTransfer
  ...
  const newCopy = helpers.deepClone(
    lastTransfer
      .filter(tx => tx.value > 0)
